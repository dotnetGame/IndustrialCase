...
public interface IElectrolyzerRecipeManager extends ILiquidAcceptManager {
	...
	public static final class ElectrolyzerOutput {
		 ...
		 */
		public ElectrolyzerOutput(String fluidName, int fluidAmount, Direction tankDirection) {
		...
		public FluidStack getOutput() {
			RegistryObject<Fluid> fluid = RegistryObject.of(new ResourceLocation(fluidName), ForgeRegistries.FLUIDS);
			return !fluid.isPresent() ? null : new FluidStack(fluid.get(), this.fluidAmount);
		}

		public Pair<FluidStack, Direction> getFullOutput() {
	...
	public static final class ElectrolyzerRecipe {
		...
		private ElectrolyzerOutput[] validateOutputs(ElectrolyzerOutput[] outputs) {
			...
			Set<Direction> directions = new HashSet<Direction>(outputs.length * 2, 0.5F);
